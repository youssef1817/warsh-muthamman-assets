...
def process_batch(batch_index: int, page_numbers: list[int], input_dir: str, output_dir: str, target_height: int) -> dict[str, object]:
    input_root = Path(input_dir)
    output_root = Path(output_dir)
    batch_entries: list[dict[str, object]] = []
    resample_method = get_resampling_method()
    ...
    for page_number in page_numbers:
        ...
        with Image.open(input_path) as image:
            ...
            new_w = int(round(orig_w * scale_ratio))
            
            # Resize image
            resized_image = image.resize((new_w, target_height), resample=resample_method)
            
            # Keep color indexing if input was indexed (P mode)
            # Pillow's resize of an indexed image can sometimes convert it to RGB depending on method,
            # or keep it as P. Let's ensure the output is saved optimally.
...
def build_index(output_dir: Path, index_path: Path, page_numbers: list[int]) -> dict[str, object]:
    ...
    index = {
        "stage": "warsh_muthamman_png_height1929",
        "format": "png",
        "pageCount": len(pages),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resize pages to a uniform height while preserving aspect ratio."
    )
    parser.add_argument("--input-dir", default="pages/warsh_muthamman_png")
    parser.add_argument("--output-dir", default="pages/warsh_muthamman_png_height1929")
    parser.add_argument("--index", default="pages/warsh_muthamman_png_height1929_index.json")
    ...
    report = {
        "generatedAt": datetime.now(timezone.utc).isoformat(),
        "profile": f"resize_to_height_{args.target_height}",
